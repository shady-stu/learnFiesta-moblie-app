  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={styles.clearBtn} activeOpacity={0.8} onPress={handleClearOffline}>
          <Text style={styles.clearText}>
            Clear Offline
          </Text>
        </TouchableOpacity>
      </View>
      {!isOnline && ( <Text style={styles.offlineText}> Offline mode: showing saved courses </Text> )}
      {isOnline && isError && (<Text style={styles.errorText}>Failed to load courses from Firebase.</Text>)}

      <CourseTabs activeTab={activeTab}onChangeTab={setActiveTab}/>

      {filteredEnrollments.length === 0 ? (
         <View style={styles.empty}> <Text style={styles.emptyText}>{isOnline ? "No courses found": "No offline courses saved yet"}</Text>
        </View>
      ) : (
